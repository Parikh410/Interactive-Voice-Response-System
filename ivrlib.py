...
soundsdir="/opt/lectureInfo/sounds"
class ivrlib(TimeoutMixin):
    ...
    def initLogger(self):
        self.log = logging.getLogger("Lecture:%s"%self.callerid)
...
class CollectOption(ivrlib):
    ...
    def __init__(self, agi, ref = None):
        self.agi = agi
        self.ref = ref
        log.debug("ref :: " + str(self.ref))
    ...
    def _startLoop(self, *res):
        ...
        if self.tries == self.maxTries:
            if self.ref == None:
                log.debug("Exceeded maximum number of tries")
                return self.noInput(self.ref)
            else:
                return self.ref.thankyou()
        else:
            self.tries += 1
            return self.playPrompt()
